...
## using future for multithreading
...
ncores <- length(availableWorkers())
```
...
invisible({
    capture.output({
      se <- swFeatureCounts (bamfiles = bamFiles,
                        saf =  genomeSW_SAF,
                        isPairedEnd = TRUE,
                        threads = 1L,
...
schedule_jobs <- length(datList) / ncores
future.scheduling <- ifelse(schedule_jobs >= 1, 
                            floor(schedule_jobs), 1)

peaks <- future_lapply(datList, trimPeaks, 
...
## output the peaks
out <- tempdir()
write.csv(as.data.frame(unname(peaks.gr)), 
          file.path(out, "peaklist.csv"), row.names=FALSE)
...
peaks <- future_lapply(seList, callPeaks, 
...
ideo <- readRDS(system.file("extdata", "ideo.mm10.rds", 
                            package = "NADfinder"))
plotSig(ideo=ideo, grList=GRangesList(peaks), 
        mcolName="AveSig", 
        layout=list("chr18"), 
